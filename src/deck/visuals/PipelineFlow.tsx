...
export const PipelineFlow: React.FC = () => {
  return (
    <GlassCard className="p-6">
      <div className="text-[12px] text-white/70">System flow</div>
      <div className="mt-5 grid gap-3">
        {STEPS.map((s, idx) => (
          <div key={s.title} className="flex items-start gap-3">
            <div className="mt-1 h-6 w-6 flex-none border border-white/15 bg-white/5 text-center text-[11px] leading-6 text-white/70">
              {idx + 1}
            </div>
            <div className="flex-1">
              <div className="text-[12px] text-white">{s.title}</div>
              <div className="mt-1 text-[11px] leading-5 text-white/60">{s.desc}</div>
            </div>
            {idx < STEPS.length - 1 ? (
